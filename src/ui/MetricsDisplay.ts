// Metrics display system for post-game statistics
...
export class MetricsDisplay {
  ...
  private static calculateManipulationIndex(metrics: GameMetrics): number {
  ...
  private static calculateGameDuration(gameHistory: GameEvent[]): string {
    if (gameHistory.length < 2) return 'Unknown';
  ...
  private static analyzeTrustPattern(gameHistory: GameEvent[]): { pattern: string; description: string } {
    const clickEvents = gameHistory.filter(event => 
      event.type === 'tile_clicked' && event.followedAdvice !== undefined
    );
  ...
  private static generateKeyInsights(metrics: GameMetrics, gameHistory: GameEvent[]): string {
