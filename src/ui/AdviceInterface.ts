// Enhanced advice interface with Windows 95-style dialogs
import { AdviceResponse, Coordinate } from '../types';
import { AIDialogManager, AIDialogCallbacks } from './AIDialogManager';
...
export class AdviceInterface {
  private container: HTMLElement;
  private config: Required<AdviceInterfaceConfig>;
  private callbacks: AdviceCallbacks;
  private currentAdvice: AdviceResponse | null = null;
  private currentCoordinate: Coordinate | null = null;
  private dialogManager: AIDialogManager;
  private isVisible = false;
  private autoHideTimeout: number | null = null;

  // Legacy UI Elements (kept for compatibility)
  private advicePanel!: HTMLElement;
  private coordinateDisplay!: HTMLElement;
  private recommendationDisplay!: HTMLElement;
  private confidenceDisplay!: HTMLElement;
  private confidenceBar!: HTMLElement;
  private reasoningDisplay!: HTMLElement;
  private actionButtons!: HTMLElement;
  private loadingIndicator!: HTMLElement;

  constructor(
    container: HTMLElement,
    config: AdviceInterfaceConfig = {},
    callbacks: AdviceCallbacks = {}
  ) {
    ...
    const dialogCallbacks: AIDialogCallbacks = {
    ...
    this.dialogManager = new AIDialogManager(dialogCallbacks);
  ...
  private setupEventListeners(): void {
    ...
    document.addEventListener('click', (event) => {
      if (this.isVisible && !this.advicePanel.contains(event.target as Node)) {
        this.hide();
      }
    });
  }

  public async requestAdvice(coordinate: Coordinate): Promise<void> {
  ...
  private displayAdvice(advice: AdviceResponse): void {
    this.currentAdvice = advice;
    ...
    if (this.config.showConfidenceBar) {
      const fill = this.confidenceBar.querySelector('.confidence-bar-fill') as HTMLElement;
  ...
  private handleAcceptAdvice(): void {
  ...
  private handleRejectAdvice(): void {
  ...
  public getCurrentAdvice(): AdviceResponse | null {
    return this.currentAdvice;
  }

  public updateConfig(newConfig: Partial<AdviceInterfaceConfig>): void {
    this.config = { ...this.config, ...newConfig };
  }

  // Enhanced methods for Windows 95-style interactions
  public showInterruption(): void {
  ...
  public showPostMortemSequence(metrics: any, gameHistory: any[]): void {
