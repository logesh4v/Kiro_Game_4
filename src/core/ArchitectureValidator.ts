// Multi-agent architecture validation system
...
export class ArchitectureValidator {
  ...
  async validateArchitecture(): Promise<ArchitectureValidationReport> {
  ...
  private async validateAgentSeparation(): Promise<ValidationResult> {
  ...
  private async validateAPIIntegration(): Promise<ValidationResult> {
    const violations: string[] = [];
    const warnings: string[] = [];
    const recommendations: string[] = [];
    let score = 100;
  ...
  private validateResponsibilityBoundaries(): ValidationResult {
    const violations: string[] = [];
    const warnings: string[] = [];
    const recommendations: string[] = [];
    let score = 100;
  ...
  private getAgentInstances(): any {
    ...
    return {
      confidentAdvisor: (this.agentRouter as any).confidentAdvisor,
      silentAnalyst: (this.agentRouter as any).silentAnalyst,
      postMortemNarrator: (this.agentRouter as any).postMortemNarrator
    };
  }

  private implementsInterface(instance: any, requiredMethods: string[]): boolean {
  ...
  private hasDirectCrossAgentCalls(): boolean {
  ...
  private validateAPIUsagePatterns(): boolean {
  ...
  private validateConfidentAdvisorResponsibilities(): boolean {
    ...
    if (typeof instance?.provideAdvice !== 'function') return false;
  ...
  private validateSilentAnalystResponsibilities(): boolean {
  ...
  private validatePostMortemNarratorResponsibilities(): boolean {
    ...
    if (typeof instance?.generateAnalysis !== 'function') return false;
  ...
  private validateGenerativeVsDeterministicSeparation(): boolean {
    ...
    const hasBedrockIntegration = (agent: any) => {
      return agent && (
        agent.bedrockService || 
  ...
  private detectAgentFunctionalityOverlap(): boolean {
