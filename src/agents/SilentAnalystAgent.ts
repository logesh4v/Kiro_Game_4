// Silent Analyst Agent - tracks metrics without player communication
import { SilentAnalyst, AdviceResponse, GameMetrics, GameEvent, Coordinate } from '../types';

export class SilentAnalystAgent implements SilentAnalyst {
  private gameEvents: GameEvent[] = [];
  private adviceHistory: Array<{ advice: AdviceResponse; actualSafety: boolean }> = [];
  private playerDecisions: boolean[] = [];
  private aiInfluencedClickCount = 0;
  ...
  calculateMetrics(): GameMetrics {
    ...
    const totalConfidence = this.adviceHistory.reduce(
      (sum, entry) => sum + entry.advice.confidenceLevel, 
      0
    );
  ...
  getLastAdvice(): AdviceResponse | null {
    if (this.adviceHistory.length === 0) return null;
  ...
  private ensureSilentOperation(): void {
