// Post-Mortem Narrator Agent - analyzes manipulation after game loss
import { PostMortemNarrator, GameMetrics, GameEvent, AIIntegrationError } from '../types';
import { BedrockService } from '../services/BedrockService';
import { PromptTemplates } from '../services/PromptTemplates';

export class PostMortemNarratorAgent implements PostMortemNarrator {
  ...
  async generateAnalysis(metrics: GameMetrics, gameHistory: GameEvent[]): Promise<string> {
    // Validate input data
    ...
    try {
      ...
      const systemPrompt = PromptTemplates.getPostMortemNarratorSystemPrompt();
      const userPrompt = PromptTemplates.generatePostMortemPrompt(metrics, gameHistory);
  ...
  private enhanceAnalysis(aiAnalysis: string, metrics: GameMetrics, gameHistory: GameEvent[]): string {
  ...
  private calculateManipulationEffectiveness(metrics: GameMetrics): number {
    // Calculate how effectively high confidence led to compliance despite low accuracy
    if (metrics.totalAdviceRequests === 0) return 0;
  ...
  private analyzeTrustDynamics(gameHistory: GameEvent[]): { pattern: string; description: string } {
    ...
    const complianceOverTime: boolean[] = [];
    let adviceIndex = 0;
  ...
  private getTotalClicks(gameHistory: GameEvent[]): number {
    return gameHistory.filter(event => event.type === 'tile_clicked').length;
  }

  private generateBehavioralInsights(metrics: GameMetrics, gameHistory: GameEvent[]): string {
  ...
  private generateClinicalConclusion(metrics: GameMetrics, confidenceGap: number): string {
  ...
  private generateFallbackAnalysis(metrics: GameMetrics, gameHistory: GameEvent[]): string {
